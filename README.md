...
5. **Networking** – Adjust each host’s `custom.networking` block (hostname, interface name, static IP, prefix length, gateway, nameservers, or `useDHCP`). If you leave `staticIPv4 = null`, DHCP is enabled automatically; otherwise set the static values that match your hypervisor (defaults assume `ens18` on `192.168.100.0/24`).
